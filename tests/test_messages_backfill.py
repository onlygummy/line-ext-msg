...
from line_ext_msg import messages
from line_ext_msg.settings import Settings

# Top-level evaluate scripts: line-anchored full literals plus
# quote-attached fragments used in string-concatenated scripts. Nested
# JS-to-JS helpers like `const pick = (root, sel) =>` legitimately take
# two arguments and match neither shape.
_TOP_ARROW_PARAMS = re.compile(r'(?m)^\s*(?:"""\s*)?(?:async\s+)?\(\s*([^)]*?)\s*\)\s*=>')
_QUOTED_ARROW_PARAMS = re.compile(r'"\(([^)]*?)\)\s*=>')


def _check_arrows(mod):
    seen = 0
    src = inspect.getsource(mod)
    for pattern in (_TOP_ARROW_PARAMS, _QUOTED_ARROW_PARAMS):
...
def test_all_evaluate_scripts_take_single_arg():
    ...
    # declaring two params silently misaligns (first gets the object,
    # rest get undefined) and querySelector throws, surfacing as a
    # bogus 'detached' stop with zero rounds scrolled.
    import line_ext_msg.rooms as rooms_mod
    for mod in (messages, rooms_mod):
        assert _check_arrows(mod) > 0, f"{mod.__name__}: no evaluate script found"
...
def _settings(**over):
    return Settings(quiet=True, **over)


def _patch_selectors():
    old = messages.SELECTORS
    messages.SELECTORS = dict(old, message_item="msg")
    return old


def test_no_scroll_when_enough_rendered():
    page = _FakePage(start=10, total=10)
    old = _patch_selectors()
    try:
        reason = messages._scroll_to_fill(cast(Page, page), _settings(), need=5)
    finally:
        messages.SELECTORS = old
    assert reason == "need"
...
def test_scrolls_until_need_met():
    ...
    old = _patch_selectors()
    try:
        messages._scroll_to_fill(cast(Page, page), _settings(), need=15)
    finally:
        messages.SELECTORS = old
...
def test_disabled_setting_skips_scroll():
    ...
    old = _patch_selectors()
    try:
        reason = messages._scroll_to_fill(
...
def test_stops_when_count_stable_at_top():
    ...
    old = _patch_selectors()
    try:
        reason = messages._scroll_to_fill(cast(Page, page), _settings(), need=50)
    finally:
        messages.SELECTORS = old
...
def test_slow_loader_at_top_does_not_stop_early():
    ...
    old = _patch_selectors()
    try:
        reason = messages._scroll_to_fill(cast(Page, page), _settings(), need=15)
    finally:
        messages.SELECTORS = old
    assert reason == "need"
    assert page.counts["msg"] == 15
...
def test_stride_target():
    from line_ext_msg.messages import _stride_target
...
def test_scroll_finder_accepts_overlay():
    ...
    # screen ever moved while the loop concluded 'top'.
    import line_ext_msg.rooms as rooms_mod
    for mod in (messages, rooms_mod):
...
def test_reverse_box_walks_negative_to_need():
    ...
    old = _patch_selectors()
    try:
        reason = messages._scroll_to_fill(cast(Page, page), _settings(), need=30)
    finally:
        messages.SELECTORS = old
    assert reason == "need"
...
def test_probe_defaults_to_normal_world():
    ...
    old = _patch_selectors()
    try:
        messages._scroll_to_fill(cast(Page, page), _settings(), need=50)
    finally:
        messages.SELECTORS = old
    assert {a.get("direction") for a in page.scroll_args} == {1}
