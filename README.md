...
ครั้งแรก: โปรแกรมเปิด Chrome โปรไฟล์แยก `%LOCALAPPDATA%\line-chrome-debug` ให้เอง ติดตั้ง LINE แล้วล็อกอิน (Chrome 136+ บล็อก `--remote-debugging-port` บนโปรไฟล์หลักโดยตรง จึงต้องใช้โปรไฟล์แยก) ถ้ายังไม่ติดตั้ง extension โปรแกรมจะเปิดหน้าต่าง Chrome ให้ติดตั้งจากเว็บสโตร์ แล้วรอจนติดตั้งเสร็จ (ปิดหน้าต่าง Chrome เพื่อยกเลิก) จากนั้นกลับ headless ให้เอง ปกติรันแบบ headless ไม่เปิดหน้าต่าง Chrome เลย พอถึงตอนต้องสแกน QR โปรแกรมจะแคป QR จากหน้า login แล้วเปิดหน้าต่าง dialog `LINE QR` ให้สแกน (ขยาย 2 เท่า ปรับได้ด้วย `--qr-zoom` หรือ `LINE_EXT_MSG_QR_ZOOM`) โดยไม่เด้งหน้าต่าง Chrome dialog รอจนกว่าจะสแกนสำเร็จหรือปิดหน้าต่าง (ไม่มีปุ่มยกเลิก ปิดด้วยปุ่ม X) ถ้าหลังสแกนแล้ว LINE ขอรหัสยืนยัน dialog จะสลับมาแสดงรหัสนั้นให้เอาไปกรอกในมือถือ แล้วกลับไปแสดง QR อีกครั้งถ้าโฟลว์ย้อนกลับ
